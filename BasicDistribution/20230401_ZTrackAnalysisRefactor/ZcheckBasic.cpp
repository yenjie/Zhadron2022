...
void ZcheckBasic_single(int binnum=40,float ptL=20,float ptH=2000,float centL=0,float centH=90,float TptL=0,float TptH=10000)
{
   ...
   hMC_Zeta->Rebin(2);
   hpp_Zeta->Rebin(2);
   hData_bkg_Zeta->Rebin(4);
   hMC_bkg_Zeta->Rebin(4);
   hMC_Zeta_gen->Rebin(2);
   hMC_bkg_Zeta_gen->Rebin(4);
   ...
   hMC_Zphi->Rebin(2);
   hpp_Zphi->Rebin(2);
   hData_bkg_Zphi->Rebin(4);
   hMC_bkg_Zphi->Rebin(4);
   hMC_Zphi_gen->Rebin(2);
   hMC_bkg_Zphi_gen->Rebin(4);
...
int main(int argc, char *argv[]){
  ...
  style();
   ...
   file_bkgMC = TFile::Open("GraphMCBackground_0503.root","read");
