...
//         sum += fland * TMath::Gaus(x[0],xx,par[3]);
      ...
      return (par[2] * step * sum * invsq2pi +par[6]*exp(par[7]*x[0]+par[8]));
...
TH1D* ZmassAnalysis(double ptL=0,double ptH=2000,int centL=0,int centH=200)
{
   ...
   TFile *infData = new TFile("output_doubleMu.root");
   ...
   TH1D *hData = new TH1D("hData","",40,61.2,121.2);
   TH1D *hDataSame = new TH1D("hDataSame","",40,61.2,121.2);
   TH1D *hMC = new TH1D("hMC","",40,61.2,121.2);
   TH1D *hMCSame = new TH1D("hMCSame","",40,61.2,121.2);
...
//   f2->SetParameters(0,0,1,91.2,1);
   ...
   TF1 *f2 = new TF1("f2",bwfun,81.2,101.2,9);
   f2->SetParNames("Mean","Width","Area","GSigma","BkgArea","ExpCont","ExpShift");
   f2->SetParameters(91,3,1.5,1);
   f2->SetLineStyle(2);
   f2->SetLineColor(2);
